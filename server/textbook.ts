...
// (Gemini → OpenRouter → Groq → NVIDIA) -> shape into the same TextbookWorkspace
...
// (Gemini → OpenRouter → Groq → NVIDIA) with its per-provider timeouts, circuit breaker
...
// provider router (Gemini → OpenRouter → Groq → NVIDIA), falling back to the
