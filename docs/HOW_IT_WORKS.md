...
- The client (`src/lib/betterAuthClient.ts` + `syncServerSession()` in
  ...
  `syncServerSession()` is **throttled** (30s, raised only after a successful
  check) so focus/online events can't spray redundant `/api/ba/get-session`
  round-trips. The mount-time pull merge only calls `setWorkspaces` when it
  ...
  Signed-in state is surfaced as a *transient toast* ("Signed in as <email>",
  mobile-friendly `inset-x-4`, 5s) instead of a permanent marketing-header chip —
  the site header stays clean, and confirmation happens at the moment of
  signing in. The workspace header account pill still shows the green check +
  "Signed in"/email persistently, and when the server answers any authed call
...
Multer's `fileFilter` accepts a file only if its mimetype is `application/pdf`
**or** its name ends in `.pdf`; anything else is rejected before the handler,
and the post-route error middleware (`server.ts`) converts that rejection — and
any other `MulterError` — into a clean `400 { error: "Only PDF files are
supported" }` instead of a 500. On top of mimetype/extension, the handler
sniffs the **magic bytes**: a real PDF starts with `%PDF-` in the first 1024
bytes, so a non-PDF file renamed to `.pdf` gets a 400 too.
...
**The map is a real layout, not random scatter.** `src/lib/mapLayout.ts` ships a
deterministic layout engine instead of trusting AI/seed coordinates (which were
near-linear, so a unit rendered as a flat row of touching cards). The exported
`computeMapLayout` groups nodes into category columns (Foundation → Mechanism →
Core Law → Real-World App), compresses the columns to whatever categories are
actually present (unknown categories append after the canonical ones), centers
each column vertically, and sorts by `depthLevel`; `MindMapCanvas.tsx` imports
it and ignores any AI-provided coordinates entirely. The layout constants
(`CATEGORY_ORDER`, `COL_STEP_X`, `ROW_STEP_Y`) are exported alongside so the
engine is unit-testable in isolation. `fitToLayout` auto-zooms to fit the whole
unit on open/reset. `routeEdge` + `laneXOf` draw
...
- `bridge.test.ts` — **Better Auth user bridge**: a magic-link account that
  ...
  and idempotency.
- `start-server.test.ts` — **real production boot** (`startServer` → `listen` on
  an ephemeral port with `NODE_ENV=production`): `/api/health`, static SPA
  serving, SPA fallback for client routes, source-map/build-artifact 404s, and a
  clean JSON 429 (not an HTML crash page) once an AI endpoint's rate limit is
  hit. Self-skips when there's no `dist/` build (CI tests before building).
- `textbook.test.ts` — **`/api/textbook/process` end-to-end** with real PDFs
  generated by `pdf-lib`: a valid upload returns a full generated workspace,
  a file renamed to `.pdf` fails the magic-byte check, a missing file and a
  non-PDF mimetype both get clean 400s (exercising the multer error
  middleware), and lowering the daily quota produces a 429 that recovers after
  `refreshQuotaLimits()`.
- `ai-endpoints-complete.test.ts` — **`/api/node/ask` + `/api/blurting/evaluate`**:
  success responses, validation 400s, safety-blocked input returning 400 before
  any provider call, the daily chat quota (429 + refresh), and the offline
  fallback-answer branches.
- `share-link.test.ts` — **read-only share links**: the pure HMAC round-trip
  (deterministic, tamper-proof across owner/workspace), plus HTTP-level
  `POST /api/share/create` ownership enforcement and `GET /api/share/read` over a
...
- `mail-config.test.ts` — **`emailConfigured()`/transport detection** from
  `SMTP_*`/`RESEND_API_KEY` env combinations (no network).
- `smtp.test.ts` — the **Gmail SMTP transport** in `server/mail.ts`: transport
  construction, send success/failure paths, and the deliverability-reason
...
- `map-layout.test.ts` — **`computeMapLayout`**: category columns compress to
  the categories actually present, unknown categories append after canonical
  ones, depth-sorted stacking, vertical centering, and that AI-provided
...
DB-gated cache + study-group tests against a throwaway Postgres container. A
separate scheduled workflow (`.github/workflows/keepalive.yml`) pings
`https://awde.onrender.com/api/health`
